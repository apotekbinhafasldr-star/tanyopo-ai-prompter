...
**Phase 4 — UMKMpro Integration.** Auth, multi-tenant data access, onboarding, the application shell, and the design system (Phase 0); real Products, AI Marketing Blueprint generation, the Promote Wizard, Campaign drafts, and Content Studio (Phase 1); a real campaign approval workflow — Budget Guard, the Approval Center, per-channel campaign breakdowns, Analytics with manual conversion logging (Phase 2); and a real Meta (Facebook & Instagram) connector — OAuth connect/disconnect, encrypted token storage, and a campaign-launch action that only ever marks a channel campaign `ACTIVE` after Meta's own API confirms it was created (Phase 3) — are all in place. Phase 4 adds the UMKMpro AI bridge: signed (HMAC-SHA256), rate-limited `/api/v1/integrations/umkmpro/*` routes for product sync, the one-time "🚀 PROMOSIKAN DENGAN AI" handoff into `/promote?handoff=<id>`, conversion recording, and an idempotent webhook receipt log — plus a profit-aware marketing estimate (revenue − COGS − ad spend, clearly labeled as an estimate) on the product detail page. AI generation, the Meta connector, and the UMKMpro integration all show a plain "not configured" state instead of fabricating output or a fake connection when their credentials are unset. The remaining feature areas (Growth, SEO, AI Marketing chat, Billing, TikTok/X connectors) are routed but still render an honest "not built yet" state — see [docs/ROADMAP.md](docs/ROADMAP.md) for what ships in each phase. Nothing in this app fakes a working integration, a connected account, or a metric that isn't real.
