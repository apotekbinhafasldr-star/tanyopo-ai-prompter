...
**Phase 2 — Marketing Operations.** Auth, multi-tenant data access, onboarding, the application shell, and the design system (Phase 0), plus real Products, AI Marketing Blueprint generation, the Promote Wizard, Campaign drafts, and Content Studio (Phase 1) are in place — all backed by a real Anthropic-powered AI provider that shows a plain "not configured" state instead of fabricating output when no API key is set. Phase 2 adds a real campaign approval workflow: Budget Guard (hard-rejects a campaign that exceeds the tenant's spend limits), the Approval Center (Owner-only approve/reject, enforced by database policy), per-channel campaign breakdowns, and Analytics with manual conversion logging. No campaign in this app ever reaches `ACTIVE` status without a real ad platform confirming it — that's Phase 3. The remaining feature areas (Growth, SEO, AI Marketing chat, Connections, Billing) are routed but still render an honest "not built yet" state — see [docs/ROADMAP.md](docs/ROADMAP.md) for what ships in each phase. Nothing in this app fakes a working integration, a connected account, or a metric that isn't real.
