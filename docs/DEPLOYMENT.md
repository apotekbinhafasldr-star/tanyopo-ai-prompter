...
| `SUPABASE_SECRET_KEY` | No | `lib/supabase/admin.ts#createAdminClient()` returns `null`; any server-only/service-role code path must treat that as `NOT_CONFIGURED`. |
...
| `UMKMPRO_SERVICE_TOKEN` | No | UMKMpro integration endpoints (Phase 4) reject requests. |
