...
- `OPENAI_API_KEY` and `ANTHROPIC_API_KEY` (`serverEnv.ai`, see [AI_SYSTEM.md](AI_SYSTEM.md) "AI Router") are read exclusively by `lib/ai/router.ts` and the two provider adapters it instantiates (`lib/ai/openai-provider.ts`, `lib/ai/anthropic-provider.ts`) — all `import "server-only"`-guarded. No feature action file, Client Component, or route handler ever touches either key directly; a feature only ever declares a task class. Neither key is required — the app runs with zero, one, or both configured.
...
`prompter_automation_settings` defaults to `automation_mode = 'manual'` and `emergency_stop_active = false` for every new tenant. Autopilot is implemented as a **policy-gated routing decision, never an autonomous execution one** — this app has no background scheduler, so nothing ever runs unattended. Every boundary below is enforced by running code, checked at the point it actually matters rather than assumed from an earlier state, and is entirely independent of the multi-provider AI Router: which AI provider (OpenAI, Anthropic, or a fallback between them) produced the `CRITICAL`-class Optimization Agent recommendation never changes any of these checks — a recommendation is data to be validated, never a source of authority.
